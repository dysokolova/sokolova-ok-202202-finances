...
fun ICorChainDsl<FinsContext>.repoOperationCheckReadLockChange(title: String) = worker {
    this.title = title
    ...
    handle {
        FinsStubs.ERROR_OPERATION_CONCURENT_ON_CHANGE.getError(this.getText(FinsStubs.ERROR_OPERATION_CONCURENT_ON_CHANGE))?.let { this.fail(it) }
